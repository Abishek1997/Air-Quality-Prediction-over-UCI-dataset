...
def initialize_params_linear():
    ...
    w = tf.Variable(tf.truncated_normal([454, 1], mean=0.0, stddev=1.0, dtype=tf.float64))
...
def nn_regression(final_train_X,train_y,final_val_X,val_y,final_test_X,test_y):
    ...
    model = Sequential()
    model.add(Dense(454, input_dim = 454,kernel_initializer='normal',kernel_regularizer = regularizers.l2(0.00025)))
